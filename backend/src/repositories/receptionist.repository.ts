...
class ReceptionistRepository {
  ...
  async updateAppointmentStatus(id: string, trang_thai: string, ly_do_huy?: string) {
    ...
    try {
      ...
      if (rows.length > 0) {
        const appt = rows[0];
        if (trang_thai === 'hoan_thanh' && appt.phac_do_dieu_tri_id) {
          ...
          await client.query(
            ...
            [completedCount, appt.phac_do_dieu_tri_id]
          );
        }
      }
  ...
  async createBilling(maHoaDon: string, khach_hang_id: string, lich_dat_id: string, don_gia: number, goi_dich_vu_id: string) {
    ...
    try {
      ...
      const { rows: hoaDonRows } = await client.query(`
        INSERT INTO hoa_don (ma_hoa_don, khach_hang_id, loai_hoa_don, phac_do_dieu_tri_id, tong_tien_phai_tra, so_tien_da_tra, trang_thai)
        VALUES ($1, $2, 'dich_vu_don', $3, $4, 0, 'chua_thanh_toan') RETURNING id, ma_hoa_don, tong_tien_phai_tra as tong_tien_thanh_toan
      `, [maHoaDon, khach_hang_id, phacDoId, don_gia]);
  ...
  async getInvoiceById(id: string) {
    const { rows } = await pool.query('SELECT tong_tien_phai_tra as tong_tien_thanh_toan, so_tien_da_tra as da_thanh_toan, loai_hoa_don, phac_do_dieu_tri_id FROM hoa_don WHERE id = $1', [id]);
    return rows[0];
  }
  ...
  async createInvoiceDirect(invoiceData: any) {
    ...
    try {
      ...
      const { rows: hdRows } = await client.query(`
        INSERT INTO hoa_don (
          ma_hoa_don, khach_hang_id, loai_hoa_don, phac_do_dieu_tri_id, cuoc_hen_id,
          tong_tien_phai_tra, so_tien_da_tra, trang_thai, voucher_id
        ) VALUES ($1, $2, $3, $4, $5, $6, 0, 'chua_thanh_toan', $7)
        RETURNING *
      `, [
        maHoaDon,
        khach_hang_id,
  ...
  async processPaymentPartial(
    ...
    try {
      ...
      await client.query(`
        UPDATE hoa_don 
        SET so_tien_da_tra = $1, trang_thai = $2 ${ngayThanhToanSql}
        WHERE id = $3
  ...
  async createInvoiceForTreatmentPlan(invoiceData: any) {
    ...
    const { rows } = await pool.query(`
      INSERT INTO hoa_don (
        ma_hoa_don, khach_hang_id, loai_hoa_don, phac_do_dieu_tri_id,
        tong_tien_phai_tra, so_tien_da_tra, trang_thai, voucher_id
      ) VALUES ($1, $2, $3, $4, $5, 0, 'chua_thanh_toan', $6)
      RETURNING *
    `, [
      maHoaDon,
      khach_hang_id,
