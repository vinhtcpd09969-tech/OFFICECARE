...
import bookingRepo, { assertTreatmentPlanCanBookSession } from './appointment.booking.repository';
import lifecycleRepo, { updateCompletedSessionsCount } from './appointment.lifecycle.repository';
import queryRepo from './appointment.query.repository';
import { Buoi, KetQuaKiemTraDatLich } from '../domain/capacity';
import { LoaiCuocHen } from '../domain/types';

// Re-export standalone utility functions for backward compatibility
export { updateCompletedSessionsCount, assertTreatmentPlanCanBookSession };

/**
 * Unified Facade for Appointment Repositories:
 * Delegates to:
 * - AppointmentBookingRepository (Booking creation & minute capacity)
 * - AppointmentLifecycleRepository (Status transitions & queue pushes)
 * - AppointmentQueryRepository (Appointments & patient records queries)
 */
