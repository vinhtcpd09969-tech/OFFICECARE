...
import { PatientVisit } from '../../../features/doctor/api/doctor.api';
...
const VISIT_STATUS_META: Record<string, { label: string; badge: string }> = {
  ...
  khong_den: { label: 'Không đến', badge: 'bg-slate-100 dark:bg-slate-800 text-slate-500 border-slate-200/50' },
  khach_khong_den: { label: 'Không đến', badge: 'bg-slate-100 dark:bg-slate-800 text-slate-500 border-slate-200/50' },
  khach_khong_den_phat: { label: 'Không đến', badge: 'bg-slate-100 dark:bg-slate-800 text-slate-500 border-slate-200/50' },
};
const getVisitStatusMeta = (t: string) => VISIT_STATUS_META[t] || { label: t, badge: 'bg-slate-100 dark:bg-slate-800 text-slate-500 border-slate-200/50' };
          ...
          {visits.map((v) => {
            ...
            return (
              <button
                ...
                className="w-full text-left bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 rounded-2xl p-4 shadow-sm hover:shadow-md hover:border-primary/30 hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-between gap-3"
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase ${v.loai === 'KHAM' ? 'bg-indigo-50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
                  ...
                  <h4 className="text-xs font-black text-secondary dark:text-slate-100 mt-1.5 truncate">
                ...
                <span className="shrink-0 flex items-center gap-1 px-3 py-1.5 bg-slate-900 dark:bg-primary text-white dark:text-slate-950 rounded-lg text-[10px] font-black">
