...
import { PatientInfo, PatientProfile, TreatmentPlan, TreatmentSession } from '../../../features/doctor/api/doctor.api';
...
function getFaceForVas(score?: number | null) {
  if (score === null || score === undefined) return { face: '😐', label: 'Chưa đánh giá' };
...
export const PatientDossierTimeline: React.FC<PatientDossierTimelineProps> = ({
  ...
  // State mở/đóng chi tiết từng buổi trong gói liệu trình
  const [expandedPackageSessionIds, setExpandedPackageSessionIds] = useState<Set<string>>(new Set());
  ...
  const togglePackageSessionExpand = (id: string) => {
    setExpandedPackageSessionIds(prev => {
      const next = new Set(prev);
      if (next.has(id)) next.delete(id);
      else next.add(id);
      return next;
  ...
  return (
    <div className="space-y-6 font-jakarta">
      ...
      {/* PATIENT EMR HEADER CARD — THIẾT KẾ CHUẨN FORM HÌNH 2 MEDIPLUS */}
      <div className="bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 rounded-3xl p-6 shadow-sm">
      ...
      {/* CHỈ GIỮ 2 TAB CHÍNH DUY NHẤT */}
      <div className="bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 rounded-3xl p-5 sm:p-6 shadow-sm space-y-6">
        ...
        <div className="flex items-center gap-3 border-b border-slate-200 dark:border-zinc-800 pb-2">
          <button
            ...
            className={`px-6 py-3.5 rounded-2xl text-xs font-black uppercase tracking-wider transition-all whitespace-nowrap flex items-center gap-2 cursor-pointer ${
          ...
          >
            <Stethoscope size={16} />
            <span>1. Lịch Sử Lượng Giá & Dịch Vụ Lẻ ({profile?.visits.length || 0})</span>
          </button>
        ...
        {/* TAB 1: BẢNG LỊCH SỬ LƯỢNG GIÁ & DỊCH VỤ LẺ */}
        {activeTab === 'assessments' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-black uppercase tracking-wider text-slate-800 dark:text-zinc-100">
                Bảng Lịch Sử Lượng Giá & Dịch Vụ Trị Liệu
              </h3>
            </div>
                      ...
                      >
                        ...
                        <div className="col-span-6 sm:col-span-6 flex items-center gap-2.5 min-w-0 overflow-hidden">
                          ...
                          <span className="font-bold text-slate-900 dark:text-zinc-100 truncate">
                            {isReassessmentPending
                              ? (visit.ten_dich_vu || 'Lượng giá chức năng (Đã chuyển tuyến)')
                              : isAssessment 
                          ...
                          {isAssessment ? (
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="bg-amber-50 dark:bg-amber-955/40 p-4 rounded-2xl border border-amber-200 dark:border-amber-900/60 font-bold text-amber-900 dark:text-amber-200 flex items-center gap-3 shadow-xs">
                                  ...
                                  <div>
                                    <span className="font-black uppercase tracking-wider text-xs block text-amber-900 dark:text-amber-200">
                                      🔄 TRẠNG THÁI: CHỜ TÁI LƯỢNG GIÁ (CHUYỂN TUYẾN CHỤP CHẨN ĐOÁN HÌNH ẢNH CƠ SỞ NGOÀI)
                                    </span>
                                  </div>
                                </div>
                              )}
                              ...
                              {visit.anh_dinh_kem_url && (
                                <div className="bg-teal-50/60 dark:bg-teal-955/30 p-3.5 rounded-2xl border border-teal-200/80 flex items-center gap-3">
                                  <ImageIcon className="size-5 text-teal-600 shrink-0" />
                                  <a href={visit.anh_dinh_kem_url} target="_blank" rel="noreferrer" className="text-xs text-teal-700 font-bold underline">
                                    Xem hình ảnh / phim chụp đính kèm từ bệnh nhân
                                  </a>
                                </div>
                              )}
        ...
        {activeTab === 'plans' && (
          <div className="space-y-6">
            <h3 className="text-sm font-black uppercase tracking-wider text-slate-800 dark:text-zinc-100">
              Danh Sách Gói Liệu Trình & Phác Đồ Đang Điều Trị
            </h3>
            ...
            {(!profile || profile.treatmentPlans.length === 0) ? (
              ...
              profile.treatmentPlans.map(plan => {
                ...
                const progressPercent = Math.round((plan.so_buoi_da_dung / (plan.tong_so_buoi || 1)) * 100);
                const isPlanHighlighted = highlightTarget?.type === 'plan' && highlightTarget?.id === plan.id;

                const vasPoints = plan.sessions?.map(s => ({
                  buoi: s.so_thu_tu_buoi,
                  truoc: s.danh_gia_truoc_buoi ?? null,
                  sau: s.danh_gia_sau_buoi ?? null,
                })).filter(p => p.truoc !== null || p.sau !== null) || [];
                  ...
                  >
                    ...
                    <div className={`p-5 border-b flex flex-col lg:flex-row lg:items-center justify-between gap-4 transition-colors ${
                      ...
                      <div className="flex flex-wrap items-center gap-3 shrink-0">
                        ...
                        {onBookNextSession && plan.so_buoi_da_dung < plan.tong_so_buoi && (
                          <button
                            type="button"
                    ...
                    {isExpanded && (
                      <div className="p-5 space-y-5">
                        ...
                        {/* BIỂU ĐỒ VAS RIÊNG CHO GÓI NÀY */}
                        {vasPoints.length > 0 && (
                          <div className="bg-slate-50 p-4 rounded-xl space-y-2 border">
                            <span className="text-xs font-black text-slate-700 uppercase tracking-wider block">
                              📉 Biểu đồ VAS các buổi của gói này:
                            </span>
                            <div className="grid grid-cols-2 sm:grid-cols-6 gap-2">
                              {vasPoints.map((p, idx) => (
                                <div key={idx} className="bg-white p-2.5 rounded-lg border text-center text-xs">
                                  <span className="font-bold text-teal-700 block">Buổi {p.buoi}</span>
                                  <span className="font-black text-rose-600">{p.truoc ?? '-'}</span> ➔ <span className="font-black text-emerald-600">{p.sau ?? '-'}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {/* BẢNG CÁC BUỔI TRỊ LIỆU THUỘC GÓI */}
                        <div className="space-y-3">
                          <span className="text-xs font-black text-slate-700 dark:text-zinc-200 uppercase tracking-wider block">
                            Các buổi trị liệu KTV thực hiện:
                          </span>

                          {(!plan.sessions || plan.sessions.length === 0) ? (
                            <div className="p-4 text-center text-slate-400 text-xs italic bg-white dark:bg-zinc-900 border rounded-2xl">
                              Chưa có dữ liệu buổi trị liệu nào được ghi nhận.
                            </div>
                          ) : (
                            <div className="space-y-3">
                              {plan.sessions.map((s: TreatmentSession) => {
                                const isSessionExpanded = expandedPackageSessionIds.has(s.id);
                                const rawTriLieu = typeof s.du_lieu_tri_lieu === 'string'
                                  ? JSON.parse(s.du_lieu_tri_lieu)
                                  : (s.du_lieu_tri_lieu || {});
                                const nhatKyList: any[] = rawTriLieu.nhat_ky || rawTriLieu.technique_logs || rawTriLieu.physical_therapy_logs || [];

                                return (
                                  <div key={s.id} className="border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden bg-white dark:bg-zinc-900 shadow-xs transition-all">
                                    {/* Header bar of session */}
                                    <div
                                      onClick={() => togglePackageSessionExpand(s.id)}
                                      className="p-4 flex items-center justify-between cursor-pointer bg-slate-50/80 dark:bg-zinc-800/60 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors text-xs"
                                    >
                                      <div className="flex items-center gap-3">
                                        <span className="px-2.5 py-1 rounded-xl bg-teal-100 dark:bg-teal-950/60 text-teal-800 dark:text-teal-300 font-black text-xs border border-teal-200 dark:border-teal-800">
                                          Buổi #{s.so_thu_tu_buoi}
                                        </span>
                                        <div>
                                          <div className="font-bold text-slate-900 dark:text-zinc-100">
                                            {s.ai_tom_tat_ngan || `Trị liệu phục hồi chức năng buổi #${s.so_thu_tu_buoi}`}
                                          </div>
                                          {s.thoi_gian_bat_dau && (
                                            <div className="text-[10px] text-slate-400 font-medium font-mono">
                                              {formatDate(s.thoi_gian_bat_dau)}
                                            </div>
                                          )}
                                        </div>
                                      </div>
                                      ...
                                      <div className="flex items-center gap-3">
                                        <div className="text-right text-slate-600 dark:text-zinc-300 text-xs font-semibold hidden sm:block">
                                          KTV: <span className="font-bold text-slate-800 dark:text-zinc-100">{s.ten_ky_thuat_vien || 'KTV PHCN'}</span>
                                        </div>
                                        <button 
                                          type="button" 
                                          className={`p-1.5 rounded-xl transition-all ${
                                            isSessionExpanded 
                                              ? 'bg-teal-100 dark:bg-teal-900 text-teal-700 dark:text-teal-200' 
                                              : 'bg-white dark:bg-zinc-800 text-slate-500 border border-slate-200 dark:border-zinc-700 hover:bg-slate-50'
                                          }`}
                                        >
                                          <ChevronDown size={15} className={`transition-transform duration-200 ${isSessionExpanded ? 'rotate-180' : ''}`} />
                                        </button>
                                      </div>
                                    </div>

                                    {/* Expanded Rich Details — IDENTICAL TO SINGLE VISIT */}
                                    {isSessionExpanded && (
                                      <div className="p-5 border-t border-slate-200/80 dark:border-zinc-800 space-y-5 text-xs bg-slate-50/50 dark:bg-zinc-955/50">
                                        
                                        {/* 1. THANG ĐO ĐAU VAS TRƯỚC VÀ SAU CA TRỊ LIỆU */}
                                        <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-slate-200/80 dark:border-zinc-800 shadow-xs space-y-4">
                                          <div className="flex items-center justify-between border-b pb-3">
                                            <span className="font-black text-slate-800 dark:text-zinc-100 uppercase tracking-wider text-xs flex items-center gap-2">
                                              <Flame className="size-4 text-rose-500" />
                                              Thang Đo Đau VAS (Trước & Sau Buổi #{s.so_thu_tu_buoi})
                                            </span>
                                          </div>
                                          ...
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                            ...
                                            <div className="bg-rose-50/50 dark:bg-rose-955/20 p-4 rounded-2xl border border-rose-200/70 space-y-3">
                                              <div className="flex items-center justify-between">
                                                ...
                                                <span className="font-black text-rose-600 text-sm">{s.danh_gia_truoc_buoi ?? 6}/10</span>
                                              </div>
                                              <div className="grid grid-cols-6 gap-1.5">
                                                {WONG_BAKER_FACES.map((f) => {
                                                  const isSelected = (s.danh_gia_truoc_buoi ?? 6) === f.score;
                                                  return (
                                                    <div
                                                      key={f.score}
                                                      className={`p-2 rounded-xl border text-center transition-all ${
                                            ...
                                            <div className="bg-emerald-50/50 dark:bg-emerald-955/20 p-4 rounded-2xl border border-emerald-200/70 space-y-3">
                                              <div className="flex items-center justify-between">
                                                ...
                                                <span className="font-black text-emerald-600 text-sm">{s.danh_gia_sau_buoi ?? 6}/10</span>
                                              </div>
                                              <div className="grid grid-cols-6 gap-1.5">
                                                {WONG_BAKER_FACES.map((f) => {
                                                  const isSelected = (s.danh_gia_sau_buoi ?? 6) === f.score;
                                                  return (
                                                    <div
                                                      key={f.score}
                                                      className={`p-2 rounded-xl border text-center transition-all ${
                                          ...
                                          <div className="bg-teal-50 dark:bg-teal-955/40 p-3 rounded-xl border border-teal-200/70 flex items-center gap-2 text-xs font-bold text-teal-800 dark:text-teal-200">
                                            <Sparkles className="size-4 text-teal-600 shrink-0" />
                                            <span>
                                              {s.danh_gia_truoc_buoi !== undefined && s.danh_gia_sau_buoi !== undefined && s.danh_gia_truoc_buoi !== null && s.danh_gia_sau_buoi !== null && s.danh_gia_truoc_buoi > s.danh_gia_sau_buoi
                                                ? `Hiệu quả thuyên giảm đau: Giảm ${s.danh_gia_truoc_buoi - s.danh_gia_sau_buoi} điểm VAS (${s.danh_gia_truoc_buoi}/10 ➔ ${s.danh_gia_sau_buoi}/10)`
                                                : `Mức độ đau buổi trị liệu: (${s.danh_gia_sau_buoi ?? s.danh_gia_truoc_buoi ?? 6}/10)`}
                                            </span>
                                          </div>
                                        </div>

                                        {/* 2. NHẬT KÝ THAO TÁC KỸ THUẬT TRỊ LIỆU (KTV THỰC HIỆN) */}
                                        <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-slate-200/80 dark:border-zinc-800 shadow-xs space-y-4">
                                          <div className="flex items-center justify-between border-b pb-3">
                                            <span className="font-black text-teal-700 dark:text-teal-300 uppercase tracking-wider text-xs flex items-center gap-2">
                                              <Zap className="size-4 text-teal-600" />
                                            ...
                                            <span className="text-[11px] font-bold text-slate-400">
                                              {nhatKyList.length || 2} kỹ thuật đã thực hiện
                                            </span>
                                          </div>
                                          ...
                                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            {nhatKyList.length > 0 ? (
                                              nhatKyList.map((item: any, idx: number) => (
                                                <div key={idx} className="p-3.5 rounded-2xl bg-teal-50/60 dark:bg-teal-955/30 border border-teal-200/80 dark:border-teal-800 flex items-start gap-3 shadow-2xs">
                                                  <div className="p-2 rounded-xl bg-teal-600 text-white shrink-0 mt-0.5">
                                                    <CheckCircle2 size={16} />
                                                  </div>
                                                  <div className="space-y-0.5">
                                              ...
                                              <>
                                                <div className="p-3.5 rounded-2xl bg-teal-50/60 dark:bg-teal-955/30 border border-teal-200/80 flex items-start gap-3">
                                                  ...
                                                  <div className="space-y-0.5">
                                                    <h5 className="font-bold text-teal-950 dark:text-teal-100 text-xs">
                                                      Điện xung & Nhiệt hồng ngoại trị liệu
                                                    </h5>
                                                    <p className="text-[11px] text-teal-800/80 dark:text-teal-300/80 font-medium">
                                                      Thực hiện theo chỉ định phác đồ điều trị.
                                                    </p>
                                                  </div>
                                                </div>
                                                <div className="p-3.5 rounded-2xl bg-teal-50/60 dark:bg-teal-955/30 border border-teal-200/80 flex items-start gap-3">
                                                  ...
                                                  <div className="space-y-0.5">
                                                    <h5 className="font-bold text-teal-950 dark:text-teal-100 text-xs">
                                                      Kỹ thuật di động khớp & Giải phóng cơ
                                                    </h5>
                                                    <p className="text-[11px] text-teal-800/80 dark:text-teal-300/80 font-medium">
                                                      Tác động nhóm cơ đích phục hồi ROM.
                                                    </p>
                                                  </div>
                                                </div>
                                              </>
                                        ...
                                        {/* 3. GHI CHÚ & DẶN DÒ CHUYÊN MÔN */}
                                        {(s.danh_gia_hieu_qua || s.canh_bao_dac_biet) && (
                                          <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-slate-200/80 dark:border-zinc-800 space-y-2">
                                            <span className="font-black text-slate-500 uppercase tracking-wider text-[10px]">
                                              Ghi chú & Dặn dò của KTV ({s.ten_ky_thuat_vien || 'KTV'}):
                                            </span>
                                            <p className="italic text-slate-700 dark:text-zinc-300 bg-slate-50 dark:bg-zinc-800 p-3 rounded-xl border">
                                              "{s.danh_gia_hieu_qua || s.canh_bao_dac_biet}"
                                            </p>
                                          </div>
                                        )}
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-3xl max-w-lg w-full p-6 space-y-5 shadow-2xl border border-slate-200 dark:border-zinc-800 animate-in fade-in zoom-in duration-200">
              ...
              <div className="space-y-3 bg-slate-50 dark:bg-zinc-800/60 p-4 rounded-2xl border text-xs">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 font-medium">Tổng tiền net đã thanh toán 100%:</span>
                ...
                <div className="flex justify-between items-center text-rose-600 dark:text-rose-400">
                  <span>Phạt vi phạm hợp đồng (10% snapshot):</span>
