...
export const PlanDetailModal: React.FC<PlanDetailModalProps> = ({ plan, onClose, onJumpToVisit, isInline }) => {
  ...
  const renderContent = () => (
    <>
      ...
      <div>
        ...
        <p className="text-xs text-zinc-550 dark:text-zinc-400 font-semibold mt-2">
  ...
  if (isInline) {
    return (
      <div className="bg-white dark:bg-zinc-900/40 rounded-3xl border border-zinc-150 dark:border-zinc-800 p-6 space-y-6 max-h-[650px] overflow-y-auto">
        <div className="border-b border-zinc-150 dark:border-zinc-800 pb-4">
          <h3 className="text-base font-black text-secondary dark:text-zinc-100 leading-snug">{plan.ten_goi || plan.ten_dich_vu}</h3>
        </div>
        {renderContent()}
      </div>
    );
