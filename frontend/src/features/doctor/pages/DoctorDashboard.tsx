...
const QueueRow = ({ item, onAssess }: QueueRowProps) => {
  ...
  const getStatusBadge = (status: string) => {
    if (status === 'dang_kham') {
      return (
        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-amber-50 dark:bg-amber-950/20 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-900/50">
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatCard
          ...
          bgClass="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30"
        />
        <StatCard
          title="Chờ gọi khám"
          value={stats.waiting}
          ...
          bgClass="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/30"
        />
      </div>
