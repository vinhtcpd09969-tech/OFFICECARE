...
// 1 buổi trong timeline của gói liệu trình — tách khỏi khối JSX lồng sâu ~150 dòng của bản cũ.
export function SessionTimelineItem({ session, previousVasSau }: SessionTimelineItemProps) {
  ...
  return (
    <div id={`session-${session.cuoc_hen_id}`} className="relative text-secondary">
      <span
        className={`absolute -left-[33px] top-0.5 size-6 rounded-full flex items-center justify-center border-2 bg-white ${
          isCompleted ? 'border-primary text-primary shadow-sm' : 'border-zinc-300 text-zinc-400'
        }`}
      >
        {isCompleted ? <CheckCircle2 size={14} /> : <Clock size={14} />}
      </span>

      <div className="bg-zinc-50/60 hover:bg-zinc-50 rounded-2xl border border-zinc-100 p-5 transition-all duration-300 space-y-4">
        {/* Header buổi điều trị */}
        <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-zinc-150/70">
          <div>
            <h4 className="text-sm font-black text-secondary">Buổi {session.so_thu_tu_buoi}</h4>
              ...
              {session.ten_bac_si && (
                <span className="flex items-center gap-1.5">
                  {session.anh_ky_thuat_vien ? (
                    ...
                    <span className="w-5 h-5 rounded-full bg-primary/10 text-primary flex items-center justify-center text-[8px] font-black shrink-0">
          ...
          <span className="px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-primary bg-primary/10 border border-primary/20 rounded-md shrink-0">
            Đã thực hiện
          </span>
        </div>
        ...
        {!hasClinicalNote ? (
          <div className="p-4 bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-150 dark:border-zinc-800 rounded-2xl text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-stretch">
            {/* Cột trái: Ghi nhận lâm sàng */}
            <div className="flex flex-col gap-4 justify-between h-full">
              <div className="space-y-4 flex-1">
                {session.chan_doan && (
                  <div className="p-4 bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-2xl shadow-xs space-y-2">
                    ...
                    <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 mt-1">{session.chan_doan}</p>
                  </div>
                )}

                {session.ghi_chu && (
                  <div className="p-4 bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-2xl shadow-xs space-y-2 h-full flex flex-col justify-start">
                    <span className="text-[10px] font-black text-zinc-400 dark:text-zinc-550 uppercase tracking-wider flex items-center gap-1.5">
                      <FileText size={13} className="text-amber-500" /> Nhật ký &amp; Ghi chú của kỹ thuật viên
                    </span>
                    <p className="text-xs text-zinc-600 dark:text-zinc-400 italic leading-relaxed mt-1 flex-1">"{session.ghi_chu}"</p>
                  </div>
                )}
              </div>

              {session.chong_chi_dinh && (
                <div className="p-4 bg-rose-50/50 dark:bg-rose-950/10 border border-rose-100 dark:border-rose-900/30 rounded-2xl flex gap-3 text-rose-800 dark:text-rose-400 shrink-0 mt-4">
            ...
            <div className="h-full">
              {(session.vas_truoc !== null || session.vas_sau !== null) && (
                <div className="p-4 bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-2xl shadow-xs space-y-4 h-full flex flex-col justify-between">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3 bg-zinc-50 dark:bg-zinc-800/40 rounded-xl text-center">
                        <span className="text-[9px] text-zinc-400 dark:text-zinc-550 uppercase font-black tracking-wider block">Trước trị liệu</span>
                        <span className="text-2xl font-black text-zinc-700 dark:text-zinc-200 mt-1 block tabular-nums">{session.vas_truoc ?? '—'}</span>
                      </div>
                      <div className="p-3 bg-teal-50/40 dark:bg-teal-950/20 rounded-xl text-center border border-teal-100/50">
                        <span className="text-[9px] text-teal-600 dark:text-teal-400 uppercase font-black tracking-wider block">Sau trị liệu</span>
                        <span className="text-2xl font-black text-teal-600 dark:text-teal-400 mt-1 block tabular-nums">{session.vas_sau ?? '—'}</span>
                      </div>
                    </div>
                    ...
                    {session.vas_truoc !== null && session.vas_sau !== null && (
                      <div className="space-y-1.5 pt-4">
                        ...
                        <div className="h-2.5 bg-zinc-100 dark:bg-zinc-800 rounded-full relative overflow-hidden">
                          {/* Biểu diễn dải chuyển màu nhiệt */}
                    ...
                    <div className="space-y-2 pt-3 border-t border-zinc-100 dark:border-zinc-800 mt-4">
                      ...
                      {session.vas_sau !== null && (
                        <div className="flex items-start gap-2 text-[11px] text-teal-600">
