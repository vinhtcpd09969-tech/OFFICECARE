...
import { TreatmentSessionDetailBody } from './TreatmentSessionDetailBody';
    ...
    <>
      {plan.goc_kham_id && (
        ...
        >
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-black uppercase tracking-wider text-slate-400">Xuất phát từ ca lượng giá</p>
            <p className="text-sm font-bold text-secondary dark:text-slate-100">Bấm để xem chi tiết ca lượng giá đã chỉ định</p>
          </div>
        </button>
      )}
      ...
      <div>
        ...
        <p className="text-xs text-slate-550 dark:text-slate-400 font-semibold mt-2">
          {plan.bac_si_chi_dinh ? `Chuyên viên chỉ định: ${plan.bac_si_chi_dinh} • ` : ''}Kích hoạt: {formatDate(plan.thoi_gian_tao)}
        </p>
      </div>
