  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-body">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-secondary text-white fixed h-full z-20">
        <div className="p-6 font-heading font-bold text-2xl flex items-center gap-2">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-[12px] font-medium transition-all duration-200 ${
                  isActive 
                    ? 'bg-primary text-white shadow-[0_4px_14px_0_rgba(46,196,182,0.39)]' 
        ...
        <div className="p-4 mt-auto">
          <button 
            ...
            className="flex items-center gap-3 w-full px-4 py-3 text-gray-400 hover:text-red-400 hover:bg-white/10 rounded-[12px] transition-colors"
          >
            <LogOut size={20} />
            Đăng xuất
          </button>
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-secondary/80 backdrop-blur-sm z-30 lg:hidden" onClick={() => setIsMobileMenuOpen(false)}>
          <aside className="w-64 bg-secondary h-full p-4 flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={handleLogout} className="flex items-center gap-3 w-full px-4 py-3 text-gray-400 hover:text-red-400 mt-auto">
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        {/* Topbar */}
        <header className="h-20 bg-white/70 backdrop-blur-md border-b border-gray-100 sticky top-0 z-10 px-4 sm:px-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center bg-gray-50 rounded-full px-4 py-2 border border-gray-200 w-64 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
              <Search size={18} className="text-gray-400 mr-2" />
              <input 
                type="text" 
                placeholder="Tìm kiếm..." 
                className="bg-transparent border-none outline-none text-sm w-full text-secondary placeholder-gray-400"
              />
            </div>
          </div>
